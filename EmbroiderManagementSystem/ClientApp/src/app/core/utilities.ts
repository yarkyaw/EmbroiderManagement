...
import { StateStoring } from 'devextreme/ui/data_grid';
...
export class Utilities {
  ...
  public static gramToKyat(val: number): string {
    const symbols = [' ကျပ် ', ' ပဲ ', ' ရွေး ', ' ခြမ်း ', ' ပွိုင့် '];
    const ratio = 16.6;
    const paeRatio = 16;
    const yaweRatio = 8;
    const pointRatio = 10;
    const chanRatio = 5;
    let conversion = val / ratio;
    let kyat = Math.floor(conversion);

    let paeConversion = Number(((conversion - kyat) * paeRatio).toFixed(2));
    let pae = Math.floor(paeConversion);

    let yaweConversion = Number(((paeConversion - pae) * yaweRatio).toFixed(2));
    let yawe = Math.floor(yaweConversion);

    let point = Number(((yaweConversion - yawe) * pointRatio).toFixed(2));
    ...
    chan=Math.floor(point/chanRatio);
    ...
    return `${kyat > 0 ? kyat + symbols[0] : ''}${pae > 0 ? pae + symbols[1] : ''}${yawe > 0 ? yawe + symbols[2] : ''}${chan > 0 ? chan + symbols[3] : ''}${point > 0 ? point + symbols[4] : ''}`;
  }
}
