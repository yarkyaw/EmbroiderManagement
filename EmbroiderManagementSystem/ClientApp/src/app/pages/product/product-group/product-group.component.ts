...
export class ProductGroupComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private modal: NzModalService, private service: ProductService, private messageService: NzMessageService, private notificationService: NzNotificationService, private authService: AuthService) {
    ...
    this.dataSource = new CustomStore({
      ...
      load: function (loadOptions: any) {
        ...
        return service.getPaginate(params, 'group')
          .toPromise()
          .then((data: any) => {
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group({
      id: 0,
      groupCode: [null, [Validators.required], [this.isCodeDuplicate()]],
  ...
  add() {
    if (this.form.valid) {
      ...
      this.service.save(obj, 'Group').subscribe(x => {
        this.form.reset();
        this.form = this.fb.group({
          id: 0,
          groupCode: [null, [Validators.required], [this.isCodeDuplicate()]],
  ...
  edit(model: any) {
    this.form = this.fb.group({
      id: model.id,
      groupCode: [model.groupCode, [Validators.required], [this.isCodeDuplicate(model.groupCode)]],
  ...
  isNameDuplicate(val: string = null): AsyncValidatorFn {
    return (c: AbstractControl): Observable<{ [key: string]: any } | null> => {
      ...
      return this.service.hasDuplicateName('Group', c.value, val ? val : '')
        .pipe(
          map(x => {
            if (x == true) {
  ...
  isCodeDuplicate(val: string = null): AsyncValidatorFn {
    return (c: AbstractControl): Observable<{ [key: string]: any } | null> => {
      ...
      return this.service.hasDuplicateCode('Group', c.value, val ? val : '')
        .pipe(
          map(x => {
            if (x == true) {
              return { duplicate: true };
