...
// Type: EmbroideryData.Interface.IAccountManager
...
namespace EmbroideryData.Interface
{
  public interface IAccountManager
  {
    ...
    Task<(bool Succeeded, string[] Errors)> CreateRoleAsync(
    ...
    Task<List<ApplicationRole>> GetRolesLoadRelatedAsync(
      int page,
      int pageSize);

    Task<(ApplicationUser User, string[] Roles)?> GetUserAndRolesAsync(
      string userId);
    ...
    Task<(bool Succeeded, string[] Errors)> UpdatePasswordAsync(
      ApplicationUser user,
    ...
    Task<(bool Succeeded, string[] Errors)> UpdateUserAsync(
      ApplicationUser user,
      IEnumerable<string> roles);
  }
}
